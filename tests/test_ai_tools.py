...
def test_data_generator():
    ...
    df: DataFrame = data_generator.get_data_frame.copy()  # Copy DataFrame that is stored inside the DataGenerator.

    for batch_index in range(number_of_samples_for_each_class // batch_size):
        data: Tuple[np.ndarray, np.ndarray, np.ndarray] = data_generator[batch_index]  # Mel spec, instrument, pitch.

        for i in range(batch_size):
            # Get all data from DataGenerator.
            decoded_instrument: str = decode_instrument(data[1][i])
            decoded_pitch: str = decode_pitch(data[2][i])

            df_index: int = (batch_index * batch_size) + i
